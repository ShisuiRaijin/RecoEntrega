...
def carga_producto_id(id_tienda_madre):
    #el boton agregar producto no deberia aparecer si el id es "crear"
    if id_tienda_madre == "crear":
       return redirect(url_for('formulario'))
        ...
        if request.method == 'POST':
            ...
            if not extension_valida(nombre_archivo):
                ...
                producto = Producto(id_tienda, nombre_producto, descripcion_producto,
                                  ruta_imagen_producto, precio_producto)
...
def busqueda():
    ...
    if request.method=='POST':
        ...
        global tienda
        tienda =mg.extraer_n_tiendas_orden(1,'desc',buscar)
...
def prod():
    ...
    #producto=mg.extraer_productos_tienda()
